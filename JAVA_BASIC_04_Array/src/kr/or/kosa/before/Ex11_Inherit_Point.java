...
class Kart {
    ArrayList<Product2> cartList = new ArrayList<>();
    int allprice;
    int allbonuspoint;

    void addOneProductToCart(Product2 product2) {
        cartList.add(product2);
    }
    void summaryKart() {
        for (Product2 product2 : cartList) {
            allprice += product2.price;
            allbonuspoint += product2.bonuspoint;
        }
        System.out.println("========= 카트에 담긴 제품들 ==========");
        for (Product2 product2 : cartList) {
            System.out.println(product2.toString());
        }
        System.out.println("담긴 총 금액" + allprice);
        System.out.println("담긴 물품 구매시 적립 예정 포인트 " + allbonuspoint);
    }
}
public class Ex11_Inherit_Point {
    public static void main(String[] args) {
        ...
        kart.addOneProductToCart(noteBook2);
        kart.addOneProductToCart(ktTv2);
        kart.summaryKart();
    }
}
