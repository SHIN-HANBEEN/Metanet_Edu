...
public class MemberController {
	...
	// SpringSecurity 로 대체합니다. login logout 은 주석 처리해도 됩니다.
...
//		String userid = (String)session.getAttribute("userid");
		//위의 코드는 Session 방식 로그인에서만 사용 가능하다. 
		
//		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
//		String userid = auth.getName();
		// 위 코드는 불편한 점이 있어서, 매개변수로 Principal principal 을 받아서 
		// 아래와 같이 처리하도록 Spring 은 만들어주었습니다. 
		
		if(principal == null) {
			return "member/login"; // principal 이 null 이면 로그인 하지 않음
		}
		String userid = principal.getName(); //SpringSecurity 인증 객체에서 꺼내오기
...
//		String userid = (String)session.getAttribute("userid");
		...
		//DB에 암호화 되어 있기 때문에 equals 를 사용 불가하다.
		if(userid != null && !userid.equals("")) { 
			Member member = memberService.selectMember(userid);
			model.addAttribute("member", member);
			model.addAttribute("message", "MEMBER_PW_RE");
...
//			member.setUserid((String)session.getAttribute("userid"));
			...
			String dbpw = memberService.getPassword(member.getUserid()); //인코딩된 비번
//			if(password != null && password.equals(dbpw)) {
			if(password != null && passwordEncoder.matches(password, dbpw)) {
				member.setPassword(dbpw);  // DB에 인코딩된 비밀번호로 삭제하면 된다.
				memberService.deleteMember(member);
				model.addAttribute("message", "DELETED_USER_INFO");
//				session.invalidate();//삭제되었으면 로그아웃 처리 --> Session 방식 이용안하면 더이상 필요 없음
				return "member/login";
			}else {
				model.addAttribute("message", "WRONG_PASSWORD");
				return "member/delete";
