...
//		member.setUserid("SAMPLEID");
		model.addAttribute("member", member);   // 폼 입력값 검증에 사용
		return "member/form";
	}

	@PostMapping(value="/member/insert")
	public String insertMember(@ModelAttribute @Valid Member member, BindingResult result, String csrfToken, HttpSession session, Model model) {
...
//			if(password != null && password.equals(dbpw)) {
				member.setPassword(dbpw); // DB 인코딩된 비번
