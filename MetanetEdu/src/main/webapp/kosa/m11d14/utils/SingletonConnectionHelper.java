package kr.or.kosa.m11d14.utils;

import java.sql.*;

public class SingletonConnectionHelper {
    private static Connection conn = null; //singleton 으로 Connection 으로 사용하는 것은 연습용으로 쓰는 것이다.
    //현업에서는 Connection pool 을 성능을 위해서 사용합니다. Hikari 가 가장 성능이 좋습니다.
    private SingletonConnectionHelper(){}

    private static class LazaHolder {
        private static final SingletonConnectionHelper INSTANCE = new SingletonConnectionHelper();
    }

    public static SingletonConnectionHelper getSingletonConnectoinHelper() {
        return LazaHolder.INSTANCE;
    }
